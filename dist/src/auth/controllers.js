...
export const githubLoginCallback = async (req, res) => {
    ...
    try {
        ...
        const existingUser = await db.query("SELECT * FROM users WHERE github_id = $1", [githubUserResult.id]);
        ...
        const userId = generateIdFromEntropySize(10);
        const newUser = await db.query("INSERT INTO users(id,username,github_id) VALUES($1, $2, $3) RETURNING *", [userId, githubUserResult.login, githubUserResult.id]);
        const session = await lucia.createSession(userId, {});
        const sessionCookie = lucia.createSessionCookie(session.id);
        res
            .status(302)
...
export const googleLoginCallback = async (req, res) => {
    try {
        ...
        const existingUser = await db.query("SELECT * FROM users WHERE google_id = $1", [user.sub]);
        if (existingUser.rows.length > 0) {
